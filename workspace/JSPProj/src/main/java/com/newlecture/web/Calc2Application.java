...
public class Calc2Application extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) 
		...
		//Context: 각각 다른 서블릿이 데이터를 이어갈 수 있게끔 해주는 저장소.
		//ServeletContext: a라는 서블릿에서 만들어진 결과를 저장하면 b가 그 결과를 이어서 새로운 결과를 만들어 낼 수 있는 상태 저장 공간.
		ServletContext application = request.getServletContext(); //*** 1 ***
 			
		String v_ = request.getParameter("v"); //html - form input name 
		...
		//operator가 뭔지에 따라 케이스 갈림. 
			//1. 계산 (= 누를 때)
		if(op.equals("=")) { 
			//application.getAttribute("v")의 반환이 객체이므로 Integer 래퍼로 감싸줌.
			int x = (Integer)application.getAttribute("v"); //"첫 번째 넣은 입력 값을 가져오겠다" *** 2 ***	
			int y = v; //지금 사용자가 전달한 값. 
			String operator = (String)application.getAttribute("op"); //*** 3 ***
			...
			application.setAttribute("v", v); //"첫 번째 입력값을 담아 놓겠다"
			application.setAttribute("op", op); //*** 4 ***
			//form 에 담겨온 파라미터를 application에 저장. 
			
			response.sendRedirect("calc2application.html");
		}
